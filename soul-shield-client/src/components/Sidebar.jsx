...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-64 flex-col bg-surface border-r border-border p-6">
      ...
      <nav className="flex-1 space-y-1">
        ...
        {isAdmin && adminLinks.map(link => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${
                isActive ? 'bg-warning/10 text-warning' : 'text-muted hover:bg-bg'
              }`
            }
          >
            <link.icon className="w-5 h-5" />
      ...
      <div className="pt-4 border-t border-border">
        ...
        <button
          type="button"
          onClick={logout}
          className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-muted hover:bg-danger/10 hover:text-danger transition-colors"
        >
